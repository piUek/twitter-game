...
function draw(svg, playersCount, w, h) {
  ...
  for (i = 0; i < playersCount; i++) {
    newPlayerBase = {
      type: i,
      x: getX(degrees * i, r, w / 2),
      y: getY(degrees * i, r, h / 2),
      fixed: true
    };
...
function addCounters(svg, playerKeys, w, h) {
  ...
  var counters = svg.selectAll("text")
    ...
    .attr("x", w - 300)
    .attr("y", function(d) {
      return 100 + playerKeys.indexOf(d) * 30
    })
    .text(0)
    .attr("font-size", "24px")
    .attr("fill", function(d) {
      return $.tweetGame.color(playerKeys.indexOf(d))
    })
    .attr("id", function(d) {
      return 'playerCounter' + playerKeys.indexOf(d)
    });
...
function handleIncStream(socket, svg, canvas, w, h, playerKeys) {
  ...
  socket.on('scorers', function(msg) {
    Array.prototype.forEach.call(msg.scorersArray, function(player) {
      var counter = {},
        counterVal = 0;
